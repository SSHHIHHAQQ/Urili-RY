...
// 查询字典数据详细
export async function getDictValueEnum(dictType, isDigital) {
    const resp = await request(`/api/system/dict/data/type/${dictType}`, {
        method: 'GET',
    });
...
export async function getDictSelectOption(dictType, isDigital) {
    const resp = await request(`/api/system/dict/data/type/${dictType}`, {
        method: 'GET',
    });
    if (resp.code === 200) {
