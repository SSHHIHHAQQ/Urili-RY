...
function parseArgs(argv) {
  const args = {
    ...
    buyerId: 2,
    reason: 'codex buyer portal product ui smoke',
  ...
  if (!Number.isFinite(args.buyerId) || args.buyerId <= 0) {
    throw new Error('--buyer-id must be a positive number');
  }
...
async function createDirectLogin(args, adminToken) {
  const response = await requestJson(
    'POST',
    `${args.baseUrl}/buyer/admin/buyers/${args.buyerId}/directLogin`,
    {
      token: adminToken,
  ...
  assertTrue(
    Number(response.code) === 200,
    `buyer direct-login ticket failed, code=${response.code}, msg=${response.msg}`,
  );
  const token =
    response.data?.token ||
    (response.data?.loginUrl
      ...
      : '');
  assertTrue(token, 'buyer direct-login ticket did not return a usable token');
  console.log('[ok] buyer direct-login ticket created');
  return `${args.frontendUrl}/buyer/direct-login?directLoginToken=${encodeURIComponent(token)}`;
}
...
async function runBrowserSmoke(args, directLoginUrl) {
  ...
  try {
    ...
    await page
      ...
      .catch(() => {});
    console.log('[ok] buyer portal loaded');

    const storage = await page.evaluate(() => ({
      adminToken: window.localStorage.getItem('access_token'),
      adminRefreshToken: window.localStorage.getItem('refresh_token'),
      adminExpireTime: window.localStorage.getItem('expireTime'),
      sellerToken: window.localStorage.getItem('seller_access_token'),
      sellerRefreshToken: window.localStorage.getItem('seller_refresh_token'),
      sellerExpireTime: window.localStorage.getItem('seller_expireTime'),
      buyerToken: window.localStorage.getItem('buyer_access_token'),
      buyerRefreshToken: window.localStorage.getItem('buyer_refresh_token'),
      buyerExpireTime: window.localStorage.getItem('buyer_expireTime'),
    }));
    assertTrue(
      storage.buyerToken,
      'buyer token was not persisted in buyer_access_token',
    );
    assertTrue(
      storage.buyerRefreshToken,
      'buyer refresh token was not persisted in buyer_refresh_token',
    );
    assertTrue(
      storage.buyerExpireTime,
      'buyer expire time was not persisted in buyer_expireTime',
    );
    assertTrue(
      !storage.adminToken && !storage.adminRefreshToken && !storage.adminExpireTime,
      'buyer portal wrote or reused admin token storage in a fresh browser context',
    );
    assertTrue(
      !storage.sellerToken && !storage.sellerRefreshToken && !storage.sellerExpireTime,
      'buyer portal wrote seller token storage in a fresh browser context',
    );
    console.log('[ok] buyer token storage isolated');

    await waitForText(page, '买家端', args.timeoutMs);
    await waitForText(page, '商品浏览准备', args.timeoutMs);
    await waitForText(page, '商城商品', args.timeoutMs);
    await waitForText(page, '销售价', args.timeoutMs);
    await waitForText(page, 'SKU', args.timeoutMs);
    console.log('[ok] buyer product card rendered');
    ...
    await waitForText(page, '尺寸重量', args.timeoutMs);
    ...
    for (const forbidden of [
      ...
      assertTrue(
        ...
        `buyer portal UI leaked visible field ${forbidden}`,
      );
    }
    console.log('[ok] buyer product detail modal rendered');
    ...
    console.log('[ok] buyer portal logout cleanup');
  } finally {
    if (context) {
      await context.close();
    }
...
async function main() {
  ...
  console.log('[pass] buyer portal product UI smoke completed.');
}
