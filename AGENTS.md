...
- 最终前端方向：三端物理分离，形成管理端、卖家端、买家端三个前端项目；当前阶段先以 `react-ui/` 作为管理端验证和改造入口，不要立即复制三份前端。
...
- 后端业务改动只进入 `RuoYi-Vue/`。
- 当前阶段前端业务改动只进入 `react-ui/`；等账号模型、端入口、菜单域和管理端流程稳定后，再按确认方案拆出管理端、卖家端、买家端三个前端目录。
...
- 后续业务表必须独立设计，但应优先复用若依的用户、部门、角色、菜单、权限、字典、配置和日志能力。
...
- 新增后端接口时，必须同步权限标识。
- 若依后端接口按场景配置 `@PreAuthorize("@ss.hasPermi('xxx')")`、`@Log`、菜单权限 `sys_menu.perms` 和按钮权限。
...
- 平台基础能力优先复用若依 `sys_user`、`sys_role`、`sys_menu`、`sys_dict`、`sys_oper_log` 等基础表，不另起第二套账号、权限、字典或日志体系。
