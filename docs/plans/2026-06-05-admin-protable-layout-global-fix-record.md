...
- 来源商品库当前展开态有 8 个筛选字段；`lg` 断点按 4 列展示时，8 个字段刚好占满 `4 x 2`，查询动作区会被挤到单独一行。此前 9 字段在 3 列断点下出现的 `3 x 3` 问题属于同一类根因。
- 第三方仓库展开态有 8 个筛选字段；超宽屏 `xxl` 断点按 8 列展示时，8 个字段刚好占满一整行，查询动作区被挤到下一行。该页面本身已使用全局 `getPersistedProTableSearch(...)`，问题属于全局查询布局缺少字段数量感知，页面字段数量刚好触发。
...
- `react-ui/src/utils/proTableSearch.ts`
  ...
  - 将 `lg` 筛选断点从 3 列调整为 4 列，降低中屏字段横向挤压；字段数固定的页面继续通过 `fieldCount` 做动作区避让。
  - 新增 `fieldCount` 配置和 `getActionSafeProTableSearchSpan(fieldCount)`，字段数固定的页面可由全局工具选择不整除字段总数的断点列数，避免查询动作区被单独挤到下一行。
...
- 业务页面接入
  ...
  - 来源商品库、第三方仓库通过 `getPersistedProTableSearch({ fieldCount })` 声明查询字段数量，不使用页面私有 CSS 移动查询按钮。
...
- 展开态断点回归：
  - `960 x 1024`：8 个筛选字段分 4 行，动作区与最后一行字段同一行。
  - `1180 x 1024`：8 个筛选字段分 3 行，动作区与最后一行字段同一行。
  - `1280 x 1024`：8 个筛选字段分 3 行，动作区与最后一行字段同一行。
  - `1366 x 1024`：8 个筛选字段分 2 行，动作区与最后一行字段同一行。
  - 以上视口下，筛选区底部到表格卡片顶部均保留 10px 间距，未再出现动作区单独悬在表格上方的问题。

- 第三方仓库 8 字段回归：
  - `2048 x 1024`：8 个筛选字段分 2 行，动作区与最后一行「结算币种 / 状态」同一行。
  - `1180 x 1024`：8 个筛选字段分 3 行，动作区与最后一行「结算币种 / 状态」同一行。
  - 以上视口下，筛选区底部到表格卡片顶部均保留 10px 间距。
...
- 同步 12 个变更文件。
- Added: 3, Modified: 9。
- 第三方仓库与来源商品库 8 字段补充修复后再次执行 `codegraph sync .`，通过；本次同步 2 个变更代码文件。
...
- 新增 `fieldCount` 查询字段数声明规则，字段数固定且容易整行占满断点的页面通过全局工具避让查询动作区。
