...
- `EDIT_PRICE` 口径已调整为 SKU 供货价变更审核；管理端 SKU 销售价调整直接生效，不再生成价格审核单。
...
- 2026-06-09 审核详情直观化改造：
  ...
  - `EDIT_PRICE` 独立展示供货价审核总览、供货价涨价/降价数量、原新供货价区间、SKU 供货价对比和高于销售价等风险。
  - 原 `对象明细` / `审核快照` 改为 `审计对象` / `审计快照`，保留追溯能力但不再作为主审核入口。
  - 浏览器验证：`新增商品 / 新增SKU / 商品资料变更 / SKU资料变更 / 价格变更` 五个 type 均进入各自 `变更预览`，且保留 `审计快照`；干净会话控制台 error/warning 为 0。
  - 按审核员视角精简新增商品详情：删除 `新增商品审核需要看完整商品` 这类系统说明和 `审核检查清单 / 已完整` 自检内容，新增商品详情直接展示真实商品资料区块。
  - 按同一原则精简新增 SKU 详情：删除顶部说明提示，详情直接展示商品上下文和新增 SKU 表。
  - 继续清理所有 type 的说明型 Alert：商品资料变更、SKU 资料变更、价格变更和兜底预览都不再展示系统备注，主审核区只保留真实资料、变化表和价格对比。
