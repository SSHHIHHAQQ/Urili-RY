...
| `system_kind` | varchar(32) | 是 | `LINGXING_WMS` | 上游系统类型 code |
| `master_warehouse_name` | varchar(200) | 是 | 无 | 主仓名称 |
| `settlement_type` | varchar(32) | 是 | `UPSTREAM_PAYABLE` | 结算类型 |
...
| `provider` | varchar(32) | 是 | 无 | `LINGXING_WMS` |
...
| 字典类型 | 选项 code | label |
| --- | --- | --- |
| `upstream_system_kind` | `LINGXING_WMS` | 领星 WMS |
...
| `upstream_settlement_type` | `UPSTREAM_PAYABLE` / `SELF_OPERATED_RECEIVABLE` | 上游仓（应付） / 自营仓（应收） |
...
迁移时不建议继续使用旧工程的 `lingxing-wms`、`upstream-payable` 这类 hyphen code 作为数据库 code。若依当前主体类型和账号角色多为大写稳定 code，Java 枚举和 MyBatis 条件也更适合大写 code。前端只展示 label。
...
| POST | `/integration/admin/upstream-systems/lingxing-wms` | `integration:upstream:add` | 新增领星 WMS 主仓并授权 |
| PUT | `/integration/admin/upstream-systems/lingxing-wms/{connectionCode}/authorization` | `integration:upstream:auth` | 重新授权 |
| PUT | `/integration/admin/upstream-systems/lingxing-wms/{connectionCode}/info` | `integration:upstream:edit` | 编辑主仓名称、结算类型 |
