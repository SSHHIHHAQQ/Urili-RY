...
状态：已确认并完成基础版实现。2026-06-08 修正读模型生成边界：库存总览必须以商城 SKU 为基础行，来源 SKU 绑定和仓库绑定只影响库存明细状态和可售计算。同日补充三视图交互：SPU 视图、SKU 视图、仓库视图。
...
1. 页面结构参考商城商品列表，支持 SPU 视图、SKU 视图和仓库视图。
2. SPU 视图为 SPU 父行，展开一次后直接显示 SKU 分组和每个 SKU 的仓库库存明细，不再要求二次展开。
3. SKU 视图以 SKU 为主行，展开 SKU + 仓库明细行；仓库视图以 SKU + 仓库为最小行，直接平铺所有仓库库存行。
...
19. 库存总览支持配对状态筛选。`仓库未配置` 和 `来源SKU未绑定` 归为未配对；`无来源库存`、`仅来源在途` 等代表关系已配对但当前库存快照不足，不归为未配对。
...
不使用顶部大 Tabs 作为库存总览视图切换。

查询区继续使用 Ant Design Pro 原生 ProTable 搜索区，不单独做筛选块。第一版筛选项如下：

| 筛选项 | 适用视图 | 用途 |
| --- | --- | --- |
| 关键词 | SPU / SKU / 仓库 | 按商品名、SPU、SKU、仓库名或仓库编码定位库存行 |
| 库存状态 | SPU / SKU / 仓库 | 按有货、缺货、仓库未配置、来源SKU未绑定、仅来源在途等状态定位问题 |
| 配对状态 | SPU / SKU / 仓库 | 快速筛出已配对或未配对库存，未配对只包含 `仓库未配置` 和 `来源SKU未绑定` |
| 仓库类型 | SPU / SKU / 仓库 | 按官方仓、三方仓、未配置或聚合混合态筛选 |
| 仓库 | SPU / SKU / 仓库 | 从当前库存明细读模型中提取实际出现过的仓库维度，支持官方来源主仓、三方仓和占位仓库 |
| 平台总库存区间 | SPU / SKU / 仓库 | 查找平台当前库存池落在指定范围内的商品或仓库行 |
| 平台可售库存区间 | SPU / SKU / 仓库 | 查找真实可售库存落在指定范围内的商品或仓库行 |
| 平台锁定库存区间 | SPU / SKU / 仓库 | 查找订单占用较高或异常的库存行 |
| 平台在途库存区间 | SPU / SKU / 仓库 | 查找配置了平台在途库存或在途库存异常的行 |
| 来源可用库存区间 | SPU / SKU / 仓库 | 查找官方仓来源可用库存落在指定范围内的行，三方仓通常为 0 |
| 来源在途库存区间 | SPU / SKU / 仓库 | 查找官方仓来源在途库存落在指定范围内的行 |
| 更新时间区间 | SPU / SKU / 仓库 | 按平台库存行或聚合读模型更新时间筛选 |
| 来源同步时间区间 | SPU / SKU / 仓库 | 按官方仓来源库存快照时间筛选 |

默认首屏优先展示关键词、库存状态、配对状态、仓库类型和仓库。数量区间和时间区间放在 ProTable 原生展开区，避免筛选区占用过多首屏空间。配对状态只作为查询维度，不新增库存事实字段。
...
展开行展示 SKU 分组和每个 SKU 下的仓库库存明细。SPU 视图只展开一次，不在展开层内再放二次展开按钮。SKU 行本身不直接编辑库存，库存调整仍发生在 SKU + 仓库明细行。

### SKU 视图

SKU 视图以 SKU 为主行，展开后展示该 SKU 的仓库库存明细。适合按单个 SKU 定位库存问题。

### 仓库视图

仓库视图以 SKU + 仓库为最小行，直接平铺所有仓库库存行。同一 SKU 如果在 A/B/C 三个仓库有库存，则展示三行。该视图适合直接定位某个仓库库存并进行双击调整。
...
| GET | `/inventory/admin/overview/warehouse/list` | `inventory:overview:query` | SKU + 仓库事实行平铺列表 |
| GET | `/inventory/admin/overview/warehouse/options` | `inventory:overview:query` | 当前库存读模型里的仓库下拉选项 |
| GET | `/inventory/admin/overview/sku/{skuId}/warehouses` | `inventory:overview:query` | SKU + 仓库明细 |
| GET | `/inventory/admin/overview/spu/{spuId}/sku-warehouses` | `inventory:overview:query` | SPU 展开层一次返回 SKU 分组和仓库明细 |
...
| POST | `/inventory/admin/overview/adjust/batch-preview` | `inventory:overview:adjust` | SPU / SKU 聚合操作按明细行批量预览 |
| POST | `/inventory/admin/overview/adjust/batch-confirm` | `inventory:overview:adjust` | SPU / SKU 聚合操作二次确认后整批落库 |
...
1. 明细行双击编辑：继续服务快速调整单个 `SKU + 仓库` 库存事实行。
2. 操作列小弹窗：SPU、SKU、展开层 SKU 标题、展开层仓库行、仓库视图平铺行都展示“调整库存”入口。

操作列弹窗规则：

- SPU 或 SKU 主行只是聚合入口，不直接写聚合读模型；保存时必须展开为 `inventory_sku_warehouse_stock.stock_id` 明细行提交。
- 弹窗显示明细行、平台总库存、平台在途库存、平台可售库存汇总。
- 明细表显示 SKU、仓库、来源可用、来源在途、当前平台总库存、调整后平台总库存、总库存增减、当前平台在途、调整后平台在途、在途增减、平台锁定、平台可售、状态和校验信息。
- 官方仓平台总库存最大值为 `来源可用库存 - 待来源扣减`，不得超过该值；来源可用为 0 时最大值就是 0，不能保存超出值。
- 三方仓平台总库存不受来源库存上限约束，但不得小于平台锁定库存。
- 平台在途库存只允许官方仓调整，最大值为 `来源在途库存 + 待来源可用观察数量`。
- `NO_WAREHOUSE` 占位行不允许保存库存调整。
- 批量确认不允许部分成功：任一明细行不允许保存时，整批返回失败，不能先保存合法行。
- 保存成功后按受影响 SKU 和 SPU 刷新永久读模型，并为平台总库存和平台在途库存变化分别追加库存流水。
...
5. 实现只读列表和仓库明细接口。
6. 实现调整预览和确认接口。
7. 实现 React 页面，复用商品列表视图切换和 ProTable 查询区，支持 SPU / SKU / 仓库三视图。
8. 接浏览器验证：SPU 一次展开、SKU 展开、仓库平铺视图、官方仓来源主仓行、无来源占位行、在途行、双击编辑。
9. 补权限、菜单和合同测试。
