...
- 最终复查时发现：外层 `.ant-pro-query-filter-actions` 坐标虽然已经在搜索卡片内，但其内部 `.ant-form-item` 仍被全局 `.ant-form-horizontal .ant-form-item` 当普通查询字段处理，保留了一个空 label 的垂直高度，导致真实按钮元素向下溢出并压到表格上方。最终必须量按钮本体，而不能只量动作区外层容器。
...
- `react-ui/src/global.css`
  ...
  - 动作区内部 `.ant-form-item` 重置为行内 flex，高度锁定为动作区高度；动作区内部 `.ant-form-item-label` 隐藏，`.ant-form-item-control` 和 content 改为 `width: auto`，保证按钮本体不会再被空 label 推出搜索卡片。
...
- `/inventory/source-warehouse-stock`
  - `1024 x 768`：展开态搜索区 4 行，动作区位于搜索卡片右下角，搜索卡片到表格卡片间距 `8px`。
    - 按钮本体复测：`maxButtonBottom=446`，`searchBottom=456`，`tableTop=464`，按钮在搜索卡片内且高于表格。
  - `1366 x 768`：展开态搜索区 2 行，动作区位于搜索卡片右下角，搜索卡片到表格卡片间距 `8px`。
  - `1536 x 864`：展开态搜索区 2 行，动作区位于搜索卡片右下角，搜索卡片到表格卡片间距 `8px`。
    - 按钮本体复测：`maxButtonBottom=254`，`searchBottom=264`，`tableTop=272`，按钮在搜索卡片内且高于表格。
  - `2048 x 1024`：7 字段收起态保持 1 行，动作区同一行靠右，搜索卡片到表格卡片间距 `10px`。
    - 按钮本体复测：`maxButtonBottom=218`，`searchBottom=230`，`tableTop=240`，按钮在搜索卡片内且高于表格。
...
- `/product/list`
  - `1024 x 768`：来源商品库展开态搜索区 3 行，动作区位于搜索卡片右下角，搜索卡片到表格卡片间距 `8px`。
    - 按钮本体复测：`maxButtonBottom=368`，`searchBottom=378`，`tableTop=386`，按钮在搜索卡片内且高于表格。
  - `2048 x 1024`：来源商品库搜索区 2 行，动作区最后一行靠右，搜索卡片到表格卡片间距 `10px`。
    - 按钮本体复测：`maxButtonBottom=300`，`searchBottom=312`，`tableTop=322`，按钮在搜索卡片内且高于表格。
  - `1180 x 1024`：搜索区 3 行，动作区最后一行靠右，搜索卡片到表格卡片间距 `10px`。
