...
| `EDIT_PRICE` | 供货价变更审核 | SKU 供货价 | 更新对应 SKU 供货价，销售状态不变 |

页面上的 `全部` 是筛选项，不写入数据库：

```text
全部 / 新增商品 / 新增SKU / 商品资料变更 / SKU资料变更 / 供货价变更
```
...
供货价审核必须突出风险：供货价涨跌幅、是否高于当前销售价、影响 SKU 数。
...
```text
商品审核
  类型 Tabs：全部 / 新增商品 / 新增SKU / 商品资料变更 / SKU资料变更 / 供货价变更
...
| 类型 | 列表重点 |
| --- | --- |
| `NEW_PRODUCT` | SKU 数、供货价区间、币种、仓库摘要、类目 |
| `ADD_SKU` | 新增 SKU 数、新 SKU 供货价区间、仓库/来源 SKU 绑定摘要 |
...
并在 service 层做待审冲突拦截：首版同一 SPU 同一时间只允许存在一个 `PENDING` 审核单，避免资料变更和供货价变更并发覆盖。
...
- 后端单元测试：新增商品、新增 SKU、资料变更、供货价变更、驳回不生效、通过才生效。
