...
> 2026-06-09 记录层 P1 修正：本文中的 2 条 verify gate P1 已由后续 `tsconfig` / `.gitignore` / gate 测试修复并验证。本文保留历史证据，不再代表当前开放 P1。
...
- 审计方式：只读；未改业务代码
- 结论：发现 `P1 x 2`（历史发现，已关闭），`P0 x 0`
- 当前状态：后续已补齐 `tsconfig` exclude、`.gitignore` 忽略规则和 gate 测试；下文证据是 2026-06-09 当时的只读快照，不代表当前仍开放。

## P1（历史发现，已关闭）

### 1. `tsc` 当时仍会把未跟踪的 `src/.umi-undefined` 当源码参与验证，三端总门对“生成目录污染类型检查”未 fail-closed
...
- 当时证据：
  ...
  - 当时工作区实际存在未跟踪目录 `react-ui/src/.umi-undefined/`，其中含大量 `.ts/.tsx` 生成文件
...
- 当时风险：
  - 当时 gate 并没有真正隔离 accidental generated dir；`tsc` 仍可能因为陈旧或脏的 `.umi-undefined` 失败或误通过，导致三端总门结果受本机残留物影响
...
### 2. 生成目录忽略规则当时不完整，未跟踪产物会持续污染工作树，掩盖真实验证漂移

- 当时证据：
  - [react-ui/.gitignore](E:/Urili-Ruoyi/react-ui/.gitignore:32) 仅忽略 `.umi`、`.umi-production`、`.umi-test`
  - [react-ui/.gitignore](E:/Urili-Ruoyi/react-ui/.gitignore:35) 没有覆盖当时实际出现的 `src/.umi-undefined/`
  - [react-ui/.gitignore](E:/Urili-Ruoyi/react-ui/.gitignore:17) 只忽略 `coverage`，没有覆盖当时实际出现的 `test-results/`
  - [E:\Urili-Ruoyi\.gitignore](E:/Urili-Ruoyi/.gitignore:1) 没有忽略当时实际出现的 `.codegraph/` 与仓库根 `.umi-test/`
  - 当时 `git status --short` 已出现 `?? .codegraph/`、`?? .umi-test/`、`?? react-ui/src/.umi-undefined/`、`?? react-ui/test-results/`
- 当时风险：
  - 虽然其中部分目录已被 `verify-three-terminal` 的测试发现逻辑绕开，但它们当时仍持续制造未跟踪噪音，降低“工作树脏是代码变更还是生成物残留”的可判读性
