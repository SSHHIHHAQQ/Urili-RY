...
### 2026-06-11 07:51 六小时审查

- 本轮新增变化：相对 `2026-06-11 01:51` 快照，未提交主线集中在 portal self-management 的 live/seed 收口而非业务面扩张：`RuoYi-Vue/seller/src/main/java/com/ruoyi/seller/controller/SellerPortalController.java`、`RuoYi-Vue/buyer/src/main/java/com/ruoyi/buyer/controller/BuyerPortalController.java` 把账号角色查询/分配收紧为同时要求 `*:account:role:query` + `*:role:list`（编辑另需 `*:account:role:edit`），`react-ui/src/pages/Portal/Home/PortalSelfManagement.tsx` 同步把“角色”按钮与“新增角色”按钮可见性对齐到同一权限闭环；`RuoYi-Vue/ruoyi-system/src/main/java/com/ruoyi/system/domain/PortalAccountProfile.java` 改为仅对账号列表暴露端内 `accountId`，继续隐藏 `subjectId` / `terminal`；`RuoYi-Vue/sql/20260610_portal_self_management_permission_seed.sql`、`scripts/portal-self-management-sql-runner.mjs`、`react-ui/scripts/verify-portal-self-management-live.mjs`、`react-ui/scripts/verify-portal-self-management-live-write.mjs`、`react-ui/scripts/verify-portal-direct-login-live.mjs` 新增 portal 首页/18 个 root button 签名、菜单 ID 区间、component/perms、DTO 字段白名单、`/getRouters`、跨端 token/ticket 拒绝、自助审计 DTO 脱敏等 live/seed 门禁。当前工作树已不再包含上一轮的 `Channel` / `integration` / `logistics` / `Product/Review` 未提交 diff，旁路收缩为 `finance quote scheme` 4 个相关文件 + 1 份实现记录。
- 与上一轮相比的有效推进：1）端内主账号自助授权闭环从“脚本准备”推进到“controller/UI/serialization/live verifier/SQL runner 同步 fail-closed”；2）`docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md` 已补两次只读 precheck 结果，当前远端 `fenxiao` 仍是每端 `7/19` self-management perms、`6/18` root button perms，缺口与执行前提都已写清；3）本轮我只读复核 `git diff --check`（仅 LF/CRLF warning）、`node scripts\verify-three-terminal.mjs --check-manifest`、前端 `5 suites / 39 tests`、后端 `PortalSelfServiceSurfaceContractTest` + `PortalHomeProfileSerializationTest` + `SqlExecutionGuardContractTest` + `FinanceAdminRouteContractTest`（共 `90` tests）全部通过。
- 方向判断：仍符合管理端保留若依 `sys_*`、seller/buyer 端账号/角色/菜单/部门/日志/会话独立、当前阶段只收最小 self-management/portal control 模板的既定方向。当前未见 seller/buyer 端内权限回挂 `sys_*`、未见前端 `sellerId` / `buyerId` 决定数据范围、未见 `@PortalPreAuthorize/@PortalLog` 缺口、未见菜单 ID 段或权限前缀回退；并且上一轮的 channel/integration/logistics/product-review 偏航已从当前未提交工作区退出。
- 风险/问题：`P0` 未见新的串端、裸 `accountId` 反查主体、`sys_*` 复用或权限前缀退化。`P1`：1）self-management 仍停在代码级/预检级闭环，`20260610_portal_self_management_permission_seed.sql` 依旧未执行，seller/buyer live `/getInfo` / `/getRouters` / direct-login / OWNER 写闭环仍缺当前轮运行态证据；2）`docs/plans/2026-06-11-quote-scheme-value-fee-implementation-record.md` 显示 finance 旁路已对远端 `fenxiao` 执行 SQL、重启后端并做 API smoke，live 基线继续在三端主线之外变化，虽然当前未提交 diff 已缩到 finance 单一路径。`P2`：`react-ui/src/pages/Portal/Home/PortalSelfManagement.tsx` 仍为 `583` 行，虽职责未散，但后续再加字段或交互应拆分。
- 当前进度评估：三端独立账号权限框架约 `96%`。
- 较上一轮变化：`+1%`，增量主要来自 controller/UI 权限闭环对齐、`accountId` 可见但不泄露主体 scope、以及 seed/live verifier/postcheck 的 fail-closed 收口；finance quote scheme 不计入完成度。
- 剩余工作估算：约 `4%`，集中在受控执行 self-management seed、seller/buyer live `/getInfo` / `/getRouters` / direct-login / write 闭环验证、以及把剩余 finance 旁路从主线 dirty tree 完全拆开。
- 预计剩余时间：若立即冻结 finance 旁路并只收 self-management seed + live 验证，约 `2-4` 小时；若继续共用当前工作树并行推进 finance 旁路，约 `5-8` 小时。
- 下一步建议：1）优先按 `docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md` 在确认目标环境后执行 `20260610_portal_self_management_permission_seed.sql` 或等价受控 runner，并记录 postcheck；2）紧接着跑 seller/buyer 账号密码登录、`/getInfo`、`/getRouters`、direct-login、OWNER 写闭环 live 验证，只修 self-management 缺口；3）把当前剩余 `finance quote scheme` 变更从三端主线 dirty tree 分离，避免 live 基线继续被非主线动作带偏。
- 本轮快照：时间 `2026-06-11 07:51 +08:00`；`git status --short` 为 `25` 个已跟踪修改、`0` 个未跟踪、`0` 个暂存，共 `25` 个变更，较上一轮 `106` 个变更收缩 `81` 个；关键主线文件为 `RuoYi-Vue/seller/src/main/java/com/ruoyi/seller/controller/SellerPortalController.java`、`RuoYi-Vue/buyer/src/main/java/com/ruoyi/buyer/controller/BuyerPortalController.java`、`RuoYi-Vue/ruoyi-system/src/main/java/com/ruoyi/system/domain/PortalAccountProfile.java`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/domain/PortalHomeProfileSerializationTest.java`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/architecture/PortalSelfServiceSurfaceContractTest.java`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/architecture/SqlExecutionGuardContractTest.java`、`RuoYi-Vue/sql/20260610_portal_self_management_permission_seed.sql`、`scripts/portal-self-management-sql-runner.mjs`、`react-ui/scripts/verify-portal-self-management-live.mjs`、`react-ui/scripts/verify-portal-self-management-live-write.mjs`、`react-ui/scripts/verify-portal-direct-login-live.mjs`、`react-ui/src/pages/Portal/Home/PortalSelfManagement.tsx`、`react-ui/tests/portal-self-management-contract.test.ts`、`react-ui/tests/portal-self-management-live-contract.test.ts`、`react-ui/tests/portal-self-management-live-write-contract.test.ts`、`react-ui/tests/portal-direct-login-live-contract.test.ts`、`docs/reports/2026-06-10-three-terminal-portal-self-management-progress-record.md`、`docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md`；剩余旁路文件仅 `RuoYi-Vue/finance/src/main/java/com/ruoyi/finance/service/impl/QuoteSchemeServiceImpl.java`、`react-ui/src/pages/Finance/QuoteScheme/index.tsx`、`react-ui/tests/finance-quote-scheme-contract.test.ts`、`docs/plans/2026-06-11-quote-scheme-value-fee-implementation-record.md`；`git diff --shortstat` 为 `25 files changed, 1514 insertions(+), 90 deletions(-)`，较上一轮收缩约 `-35 files / -2174 insertions / -375 deletions`，当前 patch hash `69b5ad229742c076c0cbc39726e39ff2993596a5`，可作为下一轮对比摘要。

### 2026-06-12 02:24 六小时审查

- 本轮新增变化：相对 `2026-06-11 07:51` 快照，当前未提交工作区最终从 `25` 个变更增至 `29` 个，`git diff --shortstat` 从 `1514 / 90` 扩至 `2392 / 128`；最近 6 小时真正新增主要集中在 live 前门禁与执行记录，而不是再铺业务面：`docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md` 与 `docs/reports/2026-06-10-three-terminal-portal-self-management-progress-record.md` 追加了 `2026-06-12 01:33-02:18` 的确认、执行和复核记录，显示 `fenxiao` 已受控执行 `20260610_portal_self_management_permission_seed.sql`，postcheck / precheck 均达到 seller/buyer `19 / 19` self-management perms、`18 / 18` root button perms；代码侧新变更集中在 `RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/domain/PortalSelfAuditSerializationTest.java`、`react-ui/scripts/verify-portal-direct-login-live.mjs`、`react-ui/scripts/verify-portal-self-management-live.mjs`、`react-ui/scripts/verify-portal-self-management-live-write.mjs` 及其对应 contract tests，补上 live 确认门槛、响应字段白名单、`/getRouters` 收口、跨端 token/ticket 拒绝和自助审计 DTO 脱敏门禁；本轮审查过程中又浮出 `RuoYi-Vue/ruoyi-framework/src/test/java/com/ruoyi/framework/aspectj/PortalLogAspectContractTest.java` 新 diff，新增 `PortalPreAuthorizeAspect` 授权失败时仍必须保留 direct-login 结构化审计字段与兼容 `oper_param` 前缀的合同，随后 `react-ui/tests/portal-session-request.test.ts` 也新增了绝对 URL / 协议相对 URL 不得被识别成合法 portal redirect 的 4 条断言。`SellerPortalController` / `BuyerPortalController`、`PortalAccountProfile`、seed SQL 和 `portal-self-management-sql-runner.mjs` 仍是上一轮已存在的续变，不是本轮新扩面。旁路仍只剩 `RuoYi-Vue/finance/src/main/java/com/ruoyi/finance/service/impl/QuoteSchemeServiceImpl.java`、`react-ui/src/pages/Finance/QuoteScheme/index.tsx`、`react-ui/tests/finance-quote-scheme-contract.test.ts`、`docs/plans/2026-06-11-quote-scheme-value-fee-implementation-record.md` 4+1 文件，以及更早遗留的 `RuoYi-Vue/ruoyi-admin/src/main/resources/application.yml`。
- 与上一轮相比的有效推进：1）上一轮最大的 `P1` 阻塞“self-management seed 未落远端库”已在 runbook/progress 记录中关闭，当前可引用的远端状态已从 `7 / 19`、`6 / 18` 提升到 `19 / 19`、`18 / 18`，且记录了执行前确认、runner postcheck 和执行后只读复核；2）live 相关 verifier 从“只校验脚本存在”推进到“先验确认、后验白名单、跨端拒绝、DTO 脱敏、角色菜单模板 ID 区间/数量”一整套 fail-closed 门禁，新增 `PortalSelfAuditSerializationTest` 后，自助登录日志/操作日志/会话 DTO 的序列化边界也有了后端合同；3）并发浮出的 `PortalLogAspectContractTest` 把“授权失败时 direct-login 审计字段不能丢”补成静态合同，而 `portal-session-request.test.ts` 又补了 portal redirect 白名单对绝对 URL / 协议相对 URL 的拒绝断言，说明主线仍在收紧 portal 安全与审计闭环而不是扩业务面；4）本轮我只读复核 `git diff --check`（仅 LF/CRLF warning）、`cd react-ui; node scripts\\verify-three-terminal.mjs --check-manifest`、前端 `portal-direct-login-live-contract` + `portal-self-management-live-contract` + `portal-self-management-live-write-contract` + `portal-self-management-sql-runner-contract`（`4 suites / 39 tests`）、`portal-session-request.test.ts`（`1 suite / 57 tests`）以及 finance 旁路 `finance-quote-scheme-contract`（`1 suite / 6 tests`），并复跑后端 `PortalSelfServiceSurfaceContractTest` + `PortalHomeProfileSerializationTest` + `PortalSelfAuditSerializationTest` + `SqlExecutionGuardContractTest`（`91` tests）、`FinanceAdminRouteContractTest`（`2` tests）和 `PortalLogAspectContractTest`（`2` tests），均通过。
- 方向判断：仍符合“管理端保留若依 `sys_*`，seller/buyer 端账号/密码/角色/菜单/部门/日志/会话独立，当前阶段只收权限框架和最小 self-management 模板”的既定方向。当前未见 seller/buyer 端内权限回挂 `sys_*`，未见前端传 `sellerId` / `buyerId` 决定数据范围，未见 `@PortalPreAuthorize/@PortalLog` 缺口，也未见 menu ID 段或权限前缀回退；并且这轮新增工作几乎都在收紧 live 运行态门禁和审计边界，没有继续铺 seller/buyer 冻结业务菜单。需要说明的是，本轮“seed 已落库”的结论来自 runbook/progress 的新增执行记录与 postcheck 摘要，不是本次审查自行执行 DML。
- 风险/问题：`P0` 未见新的串端、裸 `accountId` 反查主体、`sys_*` 复用、权限前缀/菜单 ID 段退化或前端 caller-controlled scope。`P1`：1）虽然 SQL/seed 缺口已按记录收口，但 seller/buyer 真实账号密码登录、真实 `/getInfo` / `/getRouters`、管理端 OWNER direct-login 一次性消费、401 跳转、真实日志/会话隔离和 OWNER 写闭环仍未被当前记录重放，主线剩余风险已从“模板没落库”收敛为“live 运行态证据未补齐”；2）`finance quote scheme` 旁路仍留在同一 dirty tree，且其记录显示已经影响远端 `fenxiao` 基线，虽然不计入三端框架完成度，但仍会持续模糊主线提交/回滚边界。`P2`：1）`react-ui/src/pages/Portal/Home/PortalSelfManagement.tsx` 仍为 `583` 行，若 live 闭环后继续加字段/交互应拆分；2）`RuoYi-Vue/ruoyi-admin/src/main/resources/application.yml` 仍是更早遗留的非本轮主线改动，后续若继续共树推进会增加审查噪音。
- 当前进度评估：三端独立账号权限框架约 `98%`。
- 较上一轮变化：`+2%`，增量主要来自 self-management seed 落库与 postcheck 记录闭环、live verifier/contract fail-closed 收口，以及自助审计 DTO 序列化边界补齐；finance quote scheme 不计入完成度。
- 剩余工作估算：约 `2%`，集中在 seller/buyer 真实登录、`/getInfo`、`/getRouters`、direct-login、401 跳转、日志/会话隔离和 OWNER 写闭环 live 验证；不包含 finance、完整业务菜单和物理三前端。
- 预计剩余时间：若拿到 seller/buyer OWNER 凭据并明确确认 live 写入日志、票据、session 和测试账号/角色/部门范围，约 `2-6` 小时；若确认迟迟不到位，则主线会停在“代码/seed 已收口、live 证据待补”的 blocked 边界。
- 下一步建议：1）优先按 runbook 现有门槛执行 seller/buyer 只读 live 登录验证与管理端 OWNER direct-login live 验证，先补 `/getInfo` / `/getRouters`、跨端拒绝和回执成功证据；2）在获得明确写入确认后，再跑 self-management write live 闭环，补账号/角色/部门创建清理和真实审计证据；3）把当前剩余 `finance quote scheme` 4+1 变更从三端主线 dirty tree 分离，避免下一轮再把旁路基线漂移折算进主线审查。
- 本轮快照：时间 `2026-06-12 02:28 +08:00`；为避免本条台账自引用，下列计数和 hash 均排除 `docs/reports/three-terminal-six-hour-review-log.md` 自身。排除台账后，当前工作树为 `28` 个已跟踪修改、`0` 个未跟踪、`0` 个暂存，共 `28` 个变更，较上一轮 `25` 个变更多出 `3` 个；关键主线文件为 `docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md`、`docs/reports/2026-06-10-three-terminal-portal-self-management-progress-record.md`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/domain/PortalSelfAuditSerializationTest.java`、`RuoYi-Vue/ruoyi-framework/src/test/java/com/ruoyi/framework/aspectj/PortalLogAspectContractTest.java`、`react-ui/scripts/verify-portal-direct-login-live.mjs`、`react-ui/scripts/verify-portal-self-management-live.mjs`、`react-ui/scripts/verify-portal-self-management-live-write.mjs`、`react-ui/tests/portal-direct-login-live-contract.test.ts`、`react-ui/tests/portal-self-management-live-contract.test.ts`、`react-ui/tests/portal-self-management-live-write-contract.test.ts`、`react-ui/tests/portal-self-management-sql-runner-contract.test.ts`、`react-ui/tests/portal-session-request.test.ts`，以及仍在工作树中的 `RuoYi-Vue/seller/src/main/java/com/ruoyi/seller/controller/SellerPortalController.java`、`RuoYi-Vue/buyer/src/main/java/com/ruoyi/buyer/controller/BuyerPortalController.java`、`RuoYi-Vue/ruoyi-system/src/main/java/com/ruoyi/system/domain/PortalAccountProfile.java`、`RuoYi-Vue/sql/20260610_portal_self_management_permission_seed.sql`、`scripts/portal-self-management-sql-runner.mjs`；旁路文件仍仅 `RuoYi-Vue/finance/src/main/java/com/ruoyi/finance/service/impl/QuoteSchemeServiceImpl.java`、`react-ui/src/pages/Finance/QuoteScheme/index.tsx`、`react-ui/tests/finance-quote-scheme-contract.test.ts`、`docs/plans/2026-06-11-quote-scheme-value-fee-implementation-record.md`，另有更早遗留 `RuoYi-Vue/ruoyi-admin/src/main/resources/application.yml`；按同口径 `git diff --shortstat` 为 `28 files changed, 2412 insertions(+), 128 deletions(-)`，较上一轮增加约 `+3 files / +898 insertions / +38 deletions`，对应 patch hash `e1a7427e76b7c2a00b545d337380061da5a1362a`，可作为下一轮对比摘要。

### 2026-06-12 13:05 六小时审查

- 本轮新增变化：相对 `2026-06-12 02:24` 快照，三端主线新增集中在 `11:13-11:34` 的 live 收口与其暴露的最小补丁，而不是继续铺业务菜单：`docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md`、`docs/reports/2026-06-10-three-terminal-portal-self-management-progress-record.md`、新增 `docs/reports/2026-06-12-three-terminal-minimal-permission-completion-audit.md` 记录了 seller/buyer OWNER 账号密码登录、`/getInfo`、`/getRouters`、管理端 direct-login 一次性消费、self-management write live 全部通过；代码侧 `SellerPortalController` / `BuyerPortalController` 显式清空请求体 `sellerId` / `buyerId`，`SellerServiceImpl` / `BuyerServiceImpl` 增加 `assert*SessionAccountWithoutPage(...)` 并固定写入时以方法参数主体 ID 覆盖请求体，`PortalTokenSupport` 为普通登录 session 默认写 `direct_login=false`，`TerminalSqlIsolationContractTest` / `PortalSelfServiceSurfaceContractTest` / `PortalTokenSupportTest` / `portal-self-management-contract.test.ts` / `verify-portal-self-management-live.mjs` 同步补齐会话 DDL、DTO 泄露、分页响应兼容和 live bug 合同。与此同时，工作树重新混入 `RuoYi-Vue/integration/**`、`react-ui/src/pages/UpstreamSystem/**`、`RuoYi-Vue/sql/20260612_upstream_sync_*.sql` 以及多份 `docs/reports/2026-06-12-upstream-sync-*.md` / `docs/reviews/2026-06-12-upstream-sync-stuck-issues-review.md` 的上游同步生命周期侧线，并且侧线记录已包含远端 SQL、后端重启和 live API 验证。
- 与上一轮相比的有效推进：上一轮唯一主风险“缺 seller/buyer 真实登录、direct-login、401/日志/会话/OWNER 写闭环 live 证据”已按新增 runbook/progress/completion audit 记录关闭；我本轮独立复核 `git diff --check`（仅 LF/CRLF warning）、`cd react-ui; node scripts\verify-three-terminal.mjs --check-manifest`、前端 `portal-self-management-contract` + `upstream-system-permission-guard`（`2 suites / 12 tests`）和后端 `PortalSelfServiceSurfaceContractTest` + `TerminalSqlIsolationContractTest` + `PortalTokenSupportTest` + `SellerServiceImplTest` + `BuyerServiceImplTest`（共 `150` tests，`10` 个 reactor 模块 BUILD SUCCESS），并用 `rg` 复查 seller/buyer 生产/测试代码未恢复端内 `sys_*` 控制面，也未出现裸 `select*AccountById(accountId)` 入口。按当前权限框架目标口径，三端主线已从“代码/seed 已收口，live 待补”推进到“代码、seed、live 和最小门禁均闭环”；`upstream sync` 生命周期/任务 UI/remote 执行不计入完成度。
- 方向判断：仍符合“管理端保留若依 `sys_*`，seller/buyer 端账号/密码/角色/菜单/部门/日志/会话独立，当前阶段只搭权限框架和最小菜单/权限模板、不铺完整业务菜单”的既定方向。当前未见 seller/buyer 端内权限回挂 `sys_*`，未见前端 `sellerId` / `buyerId` 决定数据范围，`SellerPortalController` / `BuyerPortalController` 仍是 `@PortalPreAuthorize` + `@PortalLog` + `PortalSessionContext.requireSession(...)`，且 live 修复继续朝 fail-closed 收紧；需要单独说明的是，当前 dirty tree 已明显转成“已完成三端最小权限框架 + 新开的 upstream sync 生命周期侧线”并行态，后者不应继续折算进三端主线。
- 风险/问题：`P0` 未见新的串端、`sys_*` 复用、裸 `accountId` 反查主体、caller-controlled scope、缺失 `@PortalPreAuthorize/@PortalLog`、菜单 ID 段或权限前缀回退。`P1`：1）`integration/upstream sync` 侧线本轮已扩到 `20` 个已跟踪文件 + `15` 个未跟踪文件，并伴随 `fenxiao` 远端 SQL、后端重启和 live API 记录，重新把非三端主线动作混回同一 dirty tree；这不再是“噪音文档”，而是影响基线与回滚边界的 scope drift，应尽快拆线；2）`docs/reports/2026-06-12-three-terminal-minimal-permission-completion-audit.md` 将三端最小权限框架标记为完成，结论与 runbook/progress、当前代码 diff 和我本轮静态复核一致，但本轮自动审查没有重复执行 live 三脚本，live 通过结论来自 `11:34` 的新增执行记录，不是本轮重放。`P2`：1）`RuoYi-Vue/ruoyi-admin/src/main/resources/application.yml` 仍是更早遗留的非主线 diff；2）`PortalSelfManagement.tsx` 大文件问题未在本轮继续恶化，但在权限框架已完成后若再加交互就应拆分。
- 当前进度评估：三端独立账号权限框架约 `100%`。
- 较上一轮变化：`+2%`，增量全部来自 seller/buyer 真实登录、`/getInfo` / `/getRouters`、direct-login、self-management write live 闭环，以及 live 暴露的 `direct_login=false` / PageHelper 污染 / controller 主体 ID 清理等 P0/P1 收口；`upstream sync` 侧线不计入完成度。
- 剩余工作估算：按当前权限框架目标口径约 `0%`；若把“将已完成主线从 `upstream sync` / 旧 `application.yml` 漂移里拆出来”视为交付整理动作，另计约 `1-3` 小时，但不纳入权限框架完成度。
- 预计剩余时间：权限框架目标 `0` 小时；若追加主线拆线、清理 dirty tree 和提交边界收口，约 `1-3` 小时。
- 下一步建议：1）把三端最小权限框架视为已完成阶段，后续六小时审查默认改成“回归监控 + 防回退”，不再把 `upstream sync`、finance 或其他业务域进度折算进完成度；2）优先把当前 `integration/upstream sync` 代码、SQL、远端执行记录和 live 证据从三端主线 dirty tree 分离到单独分支/提交/报告；3）若继续保留同一工作树，下一轮审查应把 seller/buyer 端控制面回退、menu ID/prefix 漂移、caller-controlled scope 和 direct-login/session audit 回退作为唯一 P0/P1 入口。
- 本轮快照：时间 `2026-06-12 13:05 +08:00`；为避免本条台账自引用，下列统计继续排除 `docs/reports/three-terminal-six-hour-review-log.md`。排除台账后，当前工作树为 `37` 个已跟踪修改、`15` 个未跟踪、`0` 个暂存，共 `52` 个变更，较上一轮 `28` 个变更多出 `24` 个；其中三端主线关键文件为 `RuoYi-Vue/seller/src/main/java/com/ruoyi/seller/controller/SellerPortalController.java`、`RuoYi-Vue/buyer/src/main/java/com/ruoyi/buyer/controller/BuyerPortalController.java`、`RuoYi-Vue/seller/src/main/java/com/ruoyi/seller/service/impl/SellerServiceImpl.java`、`RuoYi-Vue/buyer/src/main/java/com/ruoyi/buyer/service/impl/BuyerServiceImpl.java`、`RuoYi-Vue/ruoyi-system/src/main/java/com/ruoyi/system/service/support/PortalTokenSupport.java`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/architecture/PortalSelfServiceSurfaceContractTest.java`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/architecture/TerminalSqlIsolationContractTest.java`、`RuoYi-Vue/ruoyi-system/src/test/java/com/ruoyi/system/service/support/PortalTokenSupportTest.java`、`RuoYi-Vue/seller/src/test/java/com/ruoyi/seller/service/impl/SellerServiceImplTest.java`、`RuoYi-Vue/buyer/src/test/java/com/ruoyi/buyer/service/impl/BuyerServiceImplTest.java`、`react-ui/scripts/verify-portal-self-management-live.mjs`、`react-ui/tests/portal-self-management-contract.test.ts`、`docs/plans/2026-06-11-three-terminal-portal-self-management-sql-live-runbook.md`、`docs/reports/2026-06-10-three-terminal-portal-self-management-progress-record.md`、`docs/reports/2026-06-12-three-terminal-minimal-permission-completion-audit.md`；侧线文件为 `RuoYi-Vue/integration/**`、`react-ui/src/pages/UpstreamSystem/**`、`react-ui/src/services/integration/**`、`react-ui/src/types/integration/upstream-system.d.ts`、`RuoYi-Vue/sql/20260612_upstream_sync_task_lifecycle.sql`、`RuoYi-Vue/sql/20260612_upstream_sync_stuck_batch_reconcile.sql`、`docs/plans/2026-06-12-upstream-sync-lifecycle-rebuild-design.md`、`docs/reports/2026-06-12-upstream-sync-*.md`、`docs/reviews/2026-06-12-upstream-sync-stuck-issues-review.md`，另有更早遗留 `RuoYi-Vue/ruoyi-admin/src/main/resources/application.yml`；按同口径 `git diff --shortstat` 为 `37 files changed, 1672 insertions(+), 62 deletions(-)`（不含 `15` 个未跟踪文件），对应 patch hash `7a84eb22721046757421bfbf681d6cd8b834c7fe`，可作为下一轮对比摘要。
